...
# tf Graph input
x = tf.compat.v1.placeholder("float", [None, n_input, 1])
y = tf.compat.v1.placeholder("float", [None, vocab_size])   #size: 112 

# RNN output node weights and biases
weights = {
    'out': tf.Variable(tf.compat.v1.random_normal([n_hidden, vocab_size]))  # n_hidden =512 , vocab_size=112
}
biases = {
    'out': tf.Variable(tf.compat.v1.random_normal([vocab_size]))
}
...
def RNN(x, weights, biases):
    ...
    # rnn 
    cell = tf.compat.v1.nn.rnn_cell.BasicLSTMCell(n_hidden)
    init_state = cell.zero_state(batch_size, dtype=tf.float32)
    # final_state 的维度是  batch * n_hidden                       --> 1 * 512
    # outputs     的维度是  batch * n_input(time_step) * n_hidden  --> 1 * 3  * 512
    outputs, final_state = tf.compat.v1.nn.dynamic_rnn(cell, x, initial_state=init_state, time_major=False)  
