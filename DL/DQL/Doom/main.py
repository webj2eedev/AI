...
class DQNetwork:
    def __init__(self, state_size, action_size, learning_rate, name='DQNetwork'):
        ...
        with tf.compat.v1.variable_scope(name):
            ...
            # [None, 84, 84, 4]
            self.inputs_ = tf.compat.v1.placeholder(tf.compat.v1.float32, [None, *state_size], name="inputs")
            self.actions_ = tf.compat.v1.placeholder(tf.compat.v1.float32, [None, 3], name="actions_")
            
            # Remember that target_Q is the R(s,a) + ymax Qhat(s', a')
            self.target_Q = tf.compat.v1.placeholder(tf.compat.v1.float32, [None], name="target")
            ...
            self.conv1_out = tf.compat.v1.nn.elu(self.conv1_batchnorm, name="conv1_out")
            ## --> [20, 20, 32]
            ...
            self.conv2_out = tf.compat.v1.nn.elu(self.conv2_batchnorm, name="conv2_out")
            ## --> [9, 9, 64]
            ...
            self.conv3_out = tf.compat.v1.nn.elu(self.conv3_batchnorm, name="conv3_out")
            ## --> [3, 3, 128]
            ...
            self.fc = tf.compat.v1.layers.dense(inputs = self.flatten,
                                  units = 512,
                                  activation = tf.compat.v1.nn.elu,
                                       kernel_initializer=tf.initializers.GlorotUniform(seed=1),
                                name="fc1")
            ...
            self.Q = tf.compat.v1.reduce_sum(tf.compat.v1.multiply(self.output, self.actions_), axis=1)
            
            
            # The loss is the difference between our predicted Q_values and the Q_target
            # Sum(Qtarget - Q)^2
            self.loss = tf.compat.v1.reduce_mean(tf.compat.v1.square(self.target_Q - self.Q))
